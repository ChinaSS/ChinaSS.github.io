/**
 config = {
    ...
    callback : function(data){}, //data:所有选中数据对象的数组
    key : {
        id : "id",
        name : "name",
        data : "data"
...
**/
define(["jquery","./treeSearch","css!InputSelect/inputSelect.css"],function($,search){
    var cache = {},
        selectEvent = "mouseup";
    ...
    Input.fn = Input.prototype = {
        ...
        dataInit : function(data){ //data 为id数组
            ...
            if(data.join(",")==this.$input.data("ids")){return;}
    ...
    Input.fn.extend({
        ...
        hidePanel : function(){
            ...
            if(!!this.config.callback){
                var obj={},objArray=[],i,length,
                    idArray = this.$input.data("ids")?this.$input.data("ids").split(","):[],
                    valArray = this.$input.val()?this.$input.val().split(","):[];
                for(i=0,length=idArray.length;i<length;i++){
                    obj[this.config.key.id]=idArray[i];
                    obj[this.config.key.name]=valArray[i];
                    objArray.push(obj);
                }
