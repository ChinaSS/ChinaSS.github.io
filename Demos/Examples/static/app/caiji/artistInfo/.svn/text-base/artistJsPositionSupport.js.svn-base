define(["jquery","app/baseSupport","UtilDir/util","CommonDir/dict"],function($,baseSupport,util,dict){
	...
	result.addPosition=function($compile,$scope){
        ...
        baseSupport.contentDialog($compile,$scope,{
			  		...
			  		buttons:[
			  		       {
			 		    	   ...
			 		    	   //close :true,//写上该属性,执行完回调函数后,自动关闭对话框
			  		    	   callback:function(dialog){
			  		    		   if($("#JsPositionInfoFormId").valid()){//如果表单验证通过
					  		    		 $scope.$apply(function (){
						  		    		   $scope.artistInfo.tempPosition.dictPostType="4";//设置职务类型为4   技术职务
						  		    		   //将新增页面接受到的值传递给职位数组
	  	  ...
	  	  template: "app/caiji/artistInfo/views/addJsPosition.html",
    ...
    var validate = function(){//职位职务校验规则 及提示信息定义
        $("#JsPositionInfoFormId").validate({//行政职位职务输入数据验证   绑定验证
            rules:{//要校验的字段及校验规则
            	orgCode : {//所属机构
            		required:true ,
            		chineseLength:32
